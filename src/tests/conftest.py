from datetime import time

import pytest

from octosynk.config import Config
from octosynk.octopus import GraphQLClient


@pytest.fixture
def config():
    return Config(
        octopus_api_key="wibble-wobble",
        octopus_device_id="0000-00-00-0000000",
        octopus_api_url="https://api.wibble.com",
        sunsynk_auth_url="https://api.sunsynk.test/oauth/",
        sunsynk_api_url="https://api.sunsynk.test/api/v1/",
        sunsynk_username="test@example.com",
        sunsynk_password="test-password",
        sunsynk_device_id="test-device-id",
        off_peak_start_time=time(23, 30),
        off_peak_end_time=time(5, 30),
        max_power_watts=8000,
    )


@pytest.fixture
def client(config):
    return GraphQLClient(config)
